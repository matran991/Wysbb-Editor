...
(function (root, factory) {
    ...
    Imgur.prototype = {
        ...
        createDragZone: function () {
            ...
            p     = this.createEls('p', {}, '');
            input = this.createEls('input', {type: 'file', accept: 'image/*',onchange: "document.getElementById('img-preview').src = window.URL.createObjectURL(this.files[0])"});

            Array.prototype.forEach.call(this.dropzone, function (zone) {
                zone.appendChild(p);
                $('.dropzone p').html('<span class="load-img-up"><img id="img-preview" src=""></span><span style="margin-left:20px"><i class="fa fa-file-image-o" aria-hidden="true" style="font-size: 40px;margin-right: 10px;"></i><i>Thả file hoặc click vào đây để upload ảnh.</i></span>');
                $('.dropzone').append('<input onchange="document.getElementById(\'img-preview\').src = window.URL.createObjectURL(this.files[0])" type="file" accept="image/*">');
                this.status(zone);
                this.upload(zone);
            }.bind(this));
        },
        ...
        matchFiles: function (file, zone) {
            ...
            if (file.type.match(/image/) && file.type !== 'image/svg+xml') {
                ...
                $('.load-img-up').addClass('load');
                $('.dropzone input').attr('disabled','disabled');
                ...
                this.post(this.endpoint, fd, function (data) {
                    $('.load-img-up').removeClass('load');
                    $('.dropzone input').removeAttr('disabled');
